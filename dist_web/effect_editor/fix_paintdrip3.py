...
# Remove the old PaintDrip class
class_pattern = r"fabric\.Image\.filters\.PaintDrip = fabric\.util\.createClass\(.*?\}\);\n"
code = re.sub(class_pattern, "", code, flags=re.DOTALL)

# Replace applyRetroEffect7
fn_pattern = r"window\.applyRetroEffect7 = function\(obj, settings, id\).*?(?=// 효과 8번)"
...
window.applyRetroEffect7 = function(obj, settings, id) {
    ...
    window.showFilterControls('물감 흘러내림', [
        ...
        { id: 'brushSize', label: '브러시 크기', min: 5, max: 150, step: 1, value: obj.retroSettings.brushSize }
    ], (pid, val) => {
    ...
    if (controlPanel) {
        ...
        btnStart.onclick = () => {
            ...
            if (paths.length > 0) {
                ...
                // Set amount to 1 if it was 0 to trigger effect
