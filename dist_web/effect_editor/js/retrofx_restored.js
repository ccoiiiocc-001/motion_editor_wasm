...
fabric.Image.filters.ImageTear = fabric.util.createClass(fabric.Image.filters.BaseFilter, {
    ...
    applyTo: function (options) {
        ...
        const h = options.imageData.height;
        const copyData = new Uint8ClampedArray(data);
        const bounds = computeMaskBounds(this.maskObj);
        if (!bounds) return;

        const hex = (this.paperColor || '#ece6dc').replace('#', '');
        const pR = parseInt(hex.substring(0, 2), 16) || 236;
        const pG = parseInt(hex.substring(2, 4), 16) || 230;
        const pB = parseInt(hex.substring(4, 6), 16) || 220;
        const gapPx = Math.max(2, this.gap);
        const vertical = Math.round(this.direction) % 2 === 0;
        const imgMinX = (bounds.minX / bounds.mw) * w;
        const imgMaxX = ((bounds.maxX + 1) / bounds.mw) * w;
        const imgMinY = (bounds.minY / bounds.mh) * h;
        const imgMaxY = ((bounds.maxY + 1) / bounds.mh) * h;
        const seed = this.seed || 1.37;
        const fiber = Math.max(0, Math.min(1, this.fiber));

        for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
                const dstIdx = (y * w + x) * 4;
                const weight = getMaskWeightInTear(this.maskObj, x, y, w, h);
                ...
                if (vertical) {
                    const line = imgMinX + (imgMaxX - imgMinX) * this.tearPos + tearEdgeNoise(y, this.jagged, seed);
                    const dist = x - line;
                    const half = gapPx * 0.5;
                    if (Math.abs(dist) <= half) {
                        const fiberN = (Math.sin(x * 0.9 + y * 0.17 + seed) * 0.5 + 0.5) * fiber * 28;
                        r = Math.min(255, pR - fiberN);
                        g = Math.min(255, pG - fiberN);
                        b = Math.min(255, pB - fiberN);
                        a = 255;
                    } else if (dist > half) {
                        const sx = x - gapPx * 0.85;
                        const edge = Math.min(1, (dist - half) / 6);
                        const shade = 1 - edge * 0.22;
                        const px = sampleBilinear(copyData, w, h, sx, y);
                        r = px[0] * shade;
                        g = px[1] * shade;
                        b = px[2] * shade;
                        a = px[3];
                    ...
                    const line = imgMinY + (imgMaxY - imgMinY) * this.tearPos + tearEdgeNoise(x, this.jagged, seed + 0.5);
                    const dist = y - line;
                    const half = gapPx * 0.5;
                    if (Math.abs(dist) <= half) {
                        const fiberN = (Math.sin(x * 0.17 + y * 0.9 + seed) * 0.5 + 0.5) * fiber * 28;
                        r = Math.min(255, pR - fiberN);
                        g = Math.min(255, pG - fiberN);
                        b = Math.min(255, pB - fiberN);
                        a = 255;
                    } else if (dist > half) {
                        const sy = y - gapPx * 0.85;
                        const edge = Math.min(1, (dist - half) / 6);
                        const shade = 1 - edge * 0.22;
...
fabric.Image.filters.HolePerspective = fabric.util.createClass(fabric.Image.filters.BaseFilter, {
    ...
    applyTo: function(options) {
        ...
        const amt = Math.max(0, Math.min(1, this.amount));
        const power = 0.35 + amt * this.depth * 0.65;
        ...
        for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
                ...
                if (weight > 0.001 && amt > 0.001 && r > 0.5) {
                    const norm = Math.min(1, r / holeR);
                    if (norm < 1) {
                        const mapped = Math.pow(norm, power) * holeR;
                        sx = cx + (dx / r) * mapped;
                        sy = cy + (dy / r) * mapped;
                        shade = 1 - (1 - norm) * this.vignette * amt;
                    } else {
                        shade = 1 - this.vignette * 0.25 * amt;
                    }
                }

                const px = sampleBilinear(copyData, w, h, sx, sy);
                const rOut = px[0] * shade;
                const gOut = px[1] * shade;
                const bOut = px[2] * shade;
                data[dstIdx] = rOut * weight + copyData[dstIdx] * (1 - weight);
                data[dstIdx + 1] = gOut * weight + copyData[dstIdx + 1] * (1 - weight);
                data[dstIdx + 2] = bOut * weight + copyData[dstIdx + 2] * (1 - weight);
                data[dstIdx + 3] = px[3] * weight + copyData[dstIdx + 3] * (1 - weight);
            }
        }
    }
...
fabric.Image.filters.PuzzleShuffle = fabric.util.createClass(fabric.Image.filters.BaseFilter, {
    ...
    applyTo: function(options) {
        ...
        const cellH = h / rows;
        const amt = Math.max(0, Math.min(1, this.amount));
        const gap = amt * this.gap;

        for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
                const dstIdx = (y * w + x) * 4;
                const weight = getMaskWeightAt(this.maskObj, x, y, w, h);
                let r = 40; let g = 40; let b = 44; let a = 255;

                if (weight > 0.001) {
                    const ci = Math.min(cols - 1, Math.floor(x / cellW));
                    const cj = Math.min(rows - 1, Math.floor(y / cellH));
                    const hash = (ci * 17 + cj * 31) % 97;
                    const ox = ((hash % 7) - 3) * gap * 0.35;
                    const oy = (((hash * 3) % 7) - 3) * gap * 0.35;
                    const rot = ((hash % 5) - 2) * this.rotate * amt;
                    const lx = x - (ci + 0.5) * cellW;
                    const ly = y - (cj + 0.5) * cellH;
                    const cos = Math.cos(rot);
                    const sin = Math.sin(rot);
                    const sx = (ci + 0.5) * cellW + lx * cos - ly * sin - ox;
                    const sy = (cj + 0.5) * cellH + lx * sin + ly * cos - oy;
...
window.applyRetroEffect6 = function(obj, settings, id) {
    ...
    if (!window.globalRetroMemory[id]) {
        window.globalRetroMemory[id] = {
            tearPos: 0.5, jagged: 18, gap: 22, direction: 0, fiber: 0.4,
            brushSize: 40, feather: 20, maskObj: null
        };
    }
    obj.retroSettings = { ...window.globalRetroMemory[id], activeFxId: id };
    ...
    if (!filter) {
        filter = new fabric.Image.filters.ImageTear({ ...obj.retroSettings, seed: 1.37 });
        obj.filters = [filter];
    }

    const syncFilter = () => {
        filter.tearPos = obj.retroSettings.tearPos;
        filter.jagged = obj.retroSettings.jagged;
        filter.gap = obj.retroSettings.gap;
        filter.direction = obj.retroSettings.direction;
        filter.fiber = obj.retroSettings.fiber;
        filter.maskObj = obj.retroSettings.maskObj;
        obj.dirty = true;
        obj.applyFilters();
        window.canvas.renderAll();
    ...
    window.showFilterControls('이미지 찢기 (영역 지정)', [
        { id: 'tearPos', label: '찢김 위치 (영역 내)', min: 0.05, max: 0.95, step: 0.01, value: obj.retroSettings.tearPos, inputType: 'range' },
        { id: 'jagged', label: '찢김 거칠기', min: 4, max: 48, step: 1, value: obj.retroSettings.jagged, inputType: 'range' },
        { id: 'gap', label: '종이 틈 (px)', min: 4, max: 60, step: 1, value: obj.retroSettings.gap, inputType: 'range' },
        { id: 'fiber', label: '섬유 질감', min: 0, max: 1, step: 0.05, value: obj.retroSettings.fiber, inputType: 'range' },
        { id: 'direction', label: '방향 (0:세로 1:가로)', min: 0, max: 1, step: 1, value: obj.retroSettings.direction, inputType: 'range' },
        { id: 'brushSize', label: '🖌️ 붓 크기', min: 5, max: 150, step: 1, value: obj.retroSettings.brushSize },
...
window.applyRetroEffect8 = function(obj, settings, id) {
    ...
    if (!window.globalRetroMemory[id]) {
        window.globalRetroMemory[id] = {
            amount: 0, holeSize: 0.45, depth: 1.2, vignette: 0.5,
            brushSize: 40, feather: 20, maskObj: null
        };
    }
    obj.retroSettings = { ...window.globalRetroMemory[id], activeFxId: id };
...
window.applyRetroEffect9 = function(obj, settings, id) {
    ...
    if (!window.globalRetroMemory[id]) {
        window.globalRetroMemory[id] = {
            amount: 0, cols: 6, rows: 4, gap: 28, rotate: 0.08,
            brushSize: 40, feather: 20, maskObj: null
        };
    }
    obj.retroSettings = { ...window.globalRetroMemory[id], activeFxId: id };
    ...
    const syncFilter = () => {
        ...
        filter.gap = obj.retroSettings.gap;
        filter.rotate = obj.retroSettings.rotate;
        filter.maskObj = obj.retroSettings.maskObj;
        obj.dirty = true;
        obj.applyFilters();
        window.canvas.renderAll();
    ...
    window.showFilterControls('퍼즐 조각화', [
        ...
        { id: 'gap', label: '조각 간격', min: 0, max: 80, step: 2, value: obj.retroSettings.gap, inputType: 'range' },
