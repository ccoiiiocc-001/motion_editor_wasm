...
function parseLrcToLines(lrcText) {
    ...
    for (const raw of lrcText.split(/\r?\n/)) {
        ...
        if (timed) {
            const text = stripStructureTagsFromLine(timed[4]);
        ...
        const text = stripStructureTagsFromLine(line);
...
function renderLineList() {
    ...
    state.lines.forEach((line, index) => {
        ...
        input.addEventListener('input', () => {
            line.text = stripStructureTagsFromLine(input.value);
            if (line.text !== input.value) input.value = line.text;
        });
...
function buildLrc() {
    ...
    sorted.forEach((line) => {
        if (line.time != null) out.push(`[${formatLrcTime(line.time)}]${line.text}`);
